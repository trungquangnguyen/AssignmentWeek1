...
class NowPlayingTableViewController: UITableViewController,UISearchBarDelegate {
   ...
   // MARK:- Private Method
    ...
    func loadMoreData(){
        self.refrestControl.beginRefreshing()
        let apiKey = "a07e22bc18f5cb106bfe4cc1f83ad8ed"
        let url = NSURL(string: "https://api.themoviedb.org/3/movie/now_playing?page=\(pageLoadMore)&api_key=\(apiKey)")
        let request = NSURLRequest(
            URL: url!,
            cachePolicy: NSURLRequestCachePolicy.ReloadIgnoringLocalCacheData,
            timeoutInterval: 10)
